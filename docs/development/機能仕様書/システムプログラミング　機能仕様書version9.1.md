...
* 管理者認証機能：管理者は、入力した管理者名とパスワードを事前にシステム内で定めた固定値と照合され認証されると、テーブル番号と該当端末の紐付けを行うことができる。<br>
    * 管理者名：固定値
    * パスワード：固定値
* テーブル番号紐付け機能: 入力された4桁の数字（0000~9999）をテーブル番号として端末に設定する。
    * テーブル番号：４桁の数字（0000~9999）
    * ※運用上のルールとして、管理者は重複しない番号を入力するものとする。
    * ※既存のテーブル番号と同じ番号が入力された場合、エラーにはせず、同一のテーブルとして操作可能。
* 例外：認証失敗・入力不備が生じている場合には、エラーメッセージを表示する。
...
〇 ユーザーは、注文カゴに入った商品を、取り消すことができる。<br>
〇 ユーザーが、注文カゴに入れた商品の状態値を「注文カゴ」にする。
##### 商品を追加
...
* ランキング表示機能：管理者は、客層ごとに7日前（168時間前）からの売上トップ5の商品を閲覧することができる。
    * 表示対象データ：分析テーブルに保存されている過去7日分（当日を含まない）のデータ。<br>
